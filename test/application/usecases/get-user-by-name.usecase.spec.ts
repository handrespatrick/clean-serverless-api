...
const makeSut = (): SutTypes => {
  ...
  const starwarsGateway = new StarwarsGatewayStub()
  const sut = new GetUserByNameUsecase(starwarsGateway)
...
describe('USECASE - GetUserByNameUsecase', () => {
  ...
  it('Should throw if user was not found', async () => {
    ...
    const promise = sut.handle('any_name')

    expect(promise).rejects.toThrow('User any_name not found')
  })
